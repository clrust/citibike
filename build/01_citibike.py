"""
Build a station-hour Citi Bike demand panel.

Defaults cover the two clean monthly windows around the October 24, 2025
e-bike speed limit: September 2025 and November 2025. Change the window with:

    python3 build/01_citibike.py --months 2025-08 2025-09 2025-11 2025-12

Expected raw files can be CSVs or ZIPs under data_raw/citi_bike/ or data_raw/.
The script searches for files containing each YYYYMM token, such as:

    data_raw/citi_bike/202509-citibike-tripdata.zip
    data_raw/202511-citibike-tripdata.csv
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--raw-dir", type=Path, default=PROJECT_ROOT / "data_raw" / "citi_bike")
    parser.add_argument("--fallback-raw-dir", type=Path, default=PROJECT_ROOT / "data_raw")
    parser.add_argument("--out", type=Path, default=PROJECT_ROOT / "data_clean" / "01_citibike.csv")
...
def find_month_files(raw_dirs: list[Path], months: list[str], city_hint: str) -> list[Path]:
    ...
    for month in months:
        ...
        for raw_dir in raw_dirs:
            ...
            jersey_city_matches.extend(
                p
                for p in month_files
                if city_hint.lower() in p.name.lower()
                and p.name.lower().startswith("jc-")
            )
            hinted_matches.extend(
                p
                for p in month_files
                if city_hint.lower() in p.name.lower()
                and not p.name.lower().startswith("jc-")
            )
        matches = hinted_matches
        if not matches:
            searched = ", ".join(str(d) for d in raw_dirs)
            if jersey_city_matches:
                jc_files = ", ".join(str(p) for p in sorted(set(jersey_city_matches)))
                raise FileNotFoundError(
                    f"Only Jersey City Citi Bike files were found for {month}: {jc_files}. "
                    "Use full NYC files like 202509-citibike-tripdata.zip."
                )
            raise FileNotFoundError(f"No Citi Bike raw file found for {month} in {searched}")
...
def standardize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    if missing:
        raise ValueError(f"Missing expected Citi Bike columns: {sorted(missing)}")
...
def build_panel(aggregates: pd.DataFrame, stations: pd.DataFrame, months: list[str]) -> pd.DataFrame:
    ...
    panel["city"] = "nyc"
    panel["system"] = "citibike"
    panel["treated_city"] = 1
...
def main() -> None:
    args = parse_args()
    files = find_month_files([args.raw_dir, args.fallback_raw_dir], args.months, "citibike")
    ...
    if not aggregates:
        raise RuntimeError("No Citi Bike trips remained after filtering to the requested months.")

    panel = build_panel(pd.concat(aggregates, ignore_index=True), pd.concat(stations, ignore_index=True), args.months)
    args.out.parent.mkdir(parents=True, exist_ok=True)
    panel.to_csv(args.out, index=False)
    print(f"Wrote {len(panel):,} Citi Bike station-hour rows to {args.out}")

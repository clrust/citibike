"""
Bootstrap the October 24, 2025 sharp-window main station-hour e-bike ATT.
...
def station_components(df: pd.DataFrame) -> pd.DataFrame:
    ...
    This uses row weights because the October sharp-window station panel is
    balanced: every retained station contributes 672 paired station-hours.
    Station weighting would therefore produce the same ATT.
    """
...
def main() -> None:
    args = parse_args()
    config = make_config(
        estimand="NYC sharp-window ATT with weather and time-slot controls",
        station_weighted=False,
        input_path=INPUT_PATH,
        results_dir=RESULTS_DIR,
        output_stem="sharp_window_bootstrap",
    ...
    out = RESULTS_DIR / "sharp_window_bootstrap_summary.csv"
